...
command line, it lands in shell history):
    ...
    .venv/bin/python scripts/smoke_tool_selector.py
...
def main() -> int:
    ...
    if not settings.tool_selector_model:
        print("Set TOOL_SELECTOR_MODEL (and TOOL_SELECTOR_API_BASE / TOOL_SELECTOR_API_KEY if self-hosted) first.", file=sys.stderr)
        return 2
