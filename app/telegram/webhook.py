...
async def claim_telegram_link(body: dict, identity: Identity = Depends(require_browser_session)):
    ...
    Two proofs meet here and neither is sufficient alone: the token proves the
    holder started this from a particular Telegram account, and the browser
    session proves which Orbit account they just signed into. Attaching
    requires both, so a stolen token can only ever add a Telegram identity to
    an account its holder can already sign into.
    """
