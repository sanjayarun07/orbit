---
name: orbit
description: "Anvaya Web3 copilot: research tokens, wallets, protocols and markets with live on-chain evidence, check a wallet's holdings, and prepare Solana or cross-chain swap quotes that the user confirms in their own wallet. Use for any crypto/DeFi question, portfolio check, or swap request."
metadata:
  mcp:
    server: orbit
...
Anvaya answers Web3 questions with live evidence and prepares trades it never
...
their own wallet, in the Anvaya web UI, never inside this conversation.
...
- `orbit_connect_wallet(session_id, address)` — attach a **public** wallet
  ...
  Anvaya never asks for or accepts a private key or seed phrase.
...
- `orbit_capabilities()`, `orbit_route_preview(request, capability, chains?)`,
  `orbit_health()` — what Anvaya can route to, and how it would.
...
reach the tools Anvaya itself discovers from MCP servers (e.g. Nansen);
...
so Anvaya separates *knowing* a wallet from *signing* with it:
...
2. **Signing stays in the user's wallet.** When a turn produces a quote, Anvaya
...
- Pass the user's words through; do not translate a question into a trade.
  "Should I buy X?" is advice and Anvaya answers it as research.
- Reuse `session_id`. Start a new session only for an unrelated topic.
- If Anvaya asks which chain a symbol is on, relay the question and send the
  user's answer back as the next `orbit_chat` message.
- Report validation warnings and evidence gaps to the user rather than
  smoothing them over; do not add figures Anvaya did not return.
